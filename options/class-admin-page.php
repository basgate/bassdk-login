	...
	public function admin_notices()
	{
		...
		if ('1' === $auth_settings['cas']) :
			...
			if (! Helper::url_is_accessible($cas_url) && ! Helper::url_is_accessible($legacy_cas_url)) :
				$authorizer_options_url = 'settings' === $auth_settings['advanced_admin_menu'] ? admin_url('options-general.php?page=basgate') : admin_url('?page=basgate');
?>
				<div class='notice notice-warning is-dismissible'>
					<p><?php esc_html_e("Can't reach CAS server. Please provide", 'basgate'); ?> <a href='<?php echo esc_attr($authorizer_options_url); ?>&tab=external'><?php esc_html_e('accurate CAS settings', 'basgate'); ?></a> <?php esc_html_e('if you intend to use it.', 'basgate'); ?></p>
				</div>
			<?php
			endif;
		endif;
