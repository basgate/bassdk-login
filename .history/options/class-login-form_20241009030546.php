...
?>
		...
		?>
			<script>
				...
				function signInCallback(res) { // jshint ignore:line
					...
					var $ = jQuery;
					console.log("Logined Successfully inside signInCallback() ", JSON.stringify(res))
					...
					if (res.hasOwnProperty('data')) {
						...
						$.post(ajaxurl, {
							...
							// alert("inside signInCallback() textStatus :" + textStatus)
							console.log("inside signInCallback() textStatus :", textStatus)
							console.log("inside signInCallback() data :", data)
	...
	public function login_form_add_external_service_links()
	{
		?>
		<script>
			alert("STARTED login_form_add_external_service_links()")
		</script>
		<?php
		// Grab plugin settings.
		$options       = Options::get_instance();
