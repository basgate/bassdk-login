...
?>
		<div id="section_info_access_login" class="section_info">
			...
			<p><?php esc_html_e('Online payment solutions for all your transactions by Basgate.', 'basgate'); ?></p>
		</div>
	<?php
	}
	...
	?>
		<fieldset>
			<label><input type="radio" id="radio_auth_settings_<?php echo esc_attr($option); ?>_external_users" name="auth_settings[<?php echo esc_attr($option); ?>]" value="external_users" <?php checked('external_users' === $auth_settings_option); ?> /> <?php esc_html_e('All authenticated users (All external service users and all WordPress users)', 'basgate'); ?></label>
			<br>
			<label><input type="radio" id="radio_auth_settings_<?php echo esc_attr($option); ?>_approved_users" name="auth_settings[<?php echo esc_attr($option); ?>]" value="approved_users" <?php checked('approved_users' === $auth_settings_option); ?> /> <?php esc_html_e('Only', 'basgate'); ?> <a href="javascript:chooseTab('access_lists' );" id="dashboard_link_approved_users"><?php esc_html_e('approved users', 'basgate'); ?></a> <?php esc_html_e('(Approved external users and all WordPress users)', 'basgate'); ?></label>
		</fieldset>
	<?php
	}
	...
	?>
		<select id="auth_settings_<?php echo esc_attr($option); ?>" name="auth_settings[<?php echo esc_attr($option); ?>]">
			<option value="---" <?php selected($auth_settings_option, '---'); ?>><?php esc_html_e("None (Don't send notification emails)", 'basgate'); ?></option>
	...
	public function print_checkbox_auth_access_should_email_approved_users($args = '')
	{
		...
		// Print option elements.
	?>
		<input type="checkbox" id="auth_settings_<?php echo esc_attr($option); ?>" name="auth_settings[<?php echo esc_attr($option); ?>]" value="1" <?php checked(1 === intval($auth_settings_option)); ?> /><label for="auth_settings_<?php echo esc_attr($option); ?>"><?php esc_html_e('Send a welcome email when approving a new user', 'basgate'); ?></label>
	<?php
	}
	...
	public function print_text_description($args = '')
	{
		...
		// Print option elements.
	?>
		<textarea rows="3" id="auth_settings_<?php echo esc_attr($option); ?>" name="auth_settings[<?php echo esc_attr($option); ?>]" value="<?php echo esc_attr($auth_settings_option); ?>" placeholder='<?php echo esc_attr(__(BasgateConstants::DESCRIPTION, "basgate")); ?>' style="width:100%">
		</textarea>
		<br />
		<small>
			<?php echo wp_kses(__('This controls the description which the user sees during checkout.', 'basgate'), Helper::$allowed_html); ?>
		</small>
	...
	public function print_text_auth_access_email_approved_users_subject($args = '')
	{
		...
		// Print option elements.
	?>
		<input type="text" id="auth_settings_<?php echo esc_attr($option); ?>" name="auth_settings[<?php echo esc_attr($option); ?>]" value="<?php echo esc_attr($auth_settings_option); ?>" placeholder="Welcome to [site_name]!" style="width:320px;" /><br /><small><?php echo wp_kses(__('You can use the <b>[site_name]</b> shortcode.', 'basgate'), Helper::$allowed_html); ?></small>
	<?php
	}
	...
	?>
		<small>
			...
			printf(
				...
				wp_kses(__('You can use %1$s, %2$s, and %3$s shortcodes.', 'basgate'), Helper::$allowed_html),
				'<b>[site_name]</b>',
				'<b>[site_url]</b>',
				'<b>[user_email]</b>'
			);
